...
-- Test that stacking two Dibujos vertically using (.-.) returns a Dibujo with 
...
-- Test that joining two Dibujos horizontally using (///) returns a Dibujo 
-- with the expected height and width
testJuntar = 
...
-- Test that superimposing two Dibujos using (^^^) returns a Dibujo with the 
...
-- Test that creating a cuarteto with four Dibujos returns a Dibujo with the 
-- expected height and width
testCuarteto = 
  TestCase $ assertEqual "Create cuarteto with four Dibujos" expected result
  where
    expected = Apilar 1.0 1.0 (Juntar 1.0 1.0 testDibujo testDibujo) 
                              (Juntar 1.0 1.0 testDibujo testDibujo)
    result = cuarteto testDibujo testDibujo testDibujo testDibujo

-- Test that superimposing a Dibujo with its four rotations using encimar4 
-- returns a Dibujo with the expected height and width
testEncimar4 = 
  TestCase $ assertEqual "Superimpose a Dibujo with its four rotations using encimar4" expected result
  where
    expected = Encimar testDibujo (Encimar (rotar testDibujo) 
                       (Encimar (r180 testDibujo) (r270 testDibujo)))
    result = encimar4 testDibujo

-- Test that creating a cyclic Dibujo with a Dibujo returns a Dibujo with the 
-- expected height and width
testCiclar = TestCase $ assertEqual "Create cyclic Dibujo with a Dibujo" expected result
  where
    expected = Apilar 1.0 1.0 (Juntar 1.0 1.0 (rotar testDibujo) testDibujo) 
                              (Juntar 1.0 1.0 (r180 testDibujo) (r270 testDibujo))
    result = ciclar testDibujo

testfoldDib1 = TestCase $ assertEqual "Test foldDib" expected result
  where
    expected = 1
    result = foldDib (const 1) id id id (\_ _ x y -> x + y :: Int) 
             (\_ _ x y -> x + y :: Int) (+) (\_ _ x -> x) testDibujo

testfoldDib2 = TestCase $ assertEqual "Correct application of foldDib on a drawing" expected result
  where
    expected = 2
    result = foldDib (const 1) id id id (\_ _ x y -> x + y :: Int) 
             (\_ _ x y -> x + y :: Int) (+) (\_ _ x -> x) (testDibujo .-. testDibujo)
...
testfoldDib3 = TestCase $ assertEqual "Transformations applied to a drawing of overlapping two copies of the same one" expected result
  where
    expected = 8
    result = foldDib (const 1) id id id (\_ _ x y -> x + y :: Int) 
             (\_ _ x y -> x + y :: Int) (+) (\_ _ x -> x) 
             (ciclar testDibujo .-. encimar4 testDibujo)

testmapDib1 = TestCase $ assertEqual "Test mapDib" expected result
  where
    expected = testDibujo
    result = mapDib figura testDibujo

testmapDib2  = TestCase $ assertEqual "Two copies of a drawing stacked on top of each other" expected result
  where
    expected = Apilar 1.0 1.0 (Rotar testDibujo) (Rotar testDibujo)
    result = mapDib (rotar.figura) (testDibujo .-. testDibujo)   
...
testmapDib3 = TestCase $ assertEqual "Correct transformation to each drawing of a list, and then a combination of those" expected result
  where
    expected = Apilar 1.0 1.0 (Juntar 1.0 1.0 (rotar (espejar testDibujo)) 
              (espejar testDibujo)) (Juntar 1.0 1.0 (r180 (espejar testDibujo)) 
              (r270 (espejar testDibujo)))
    result = mapDib (espejar.figura) (ciclar testDibujo)
